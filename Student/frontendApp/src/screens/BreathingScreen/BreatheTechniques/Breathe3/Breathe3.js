...
//Sound Timer and Animation
  ...
  const playSound = () => {
    if (!sound.current) {
      sound.current = new Sound('breathe3.mp3', Sound.MAIN_BUNDLE, (error) => {
        if (error) {
  ...
  return (
    <View style={{ height: height * 0.7, width: width }}>
      ...
      <TouchableOpacity onPress={Breathe1}> 
        <Text style={{ padding: 2, color: 'black' }}>
          <Icon name='play-back-outline' style={{ fontSize: 30 }} />
        </Text>
        </TouchableOpacity>

        <Text style={{ padding: 2, color: 'white', backgroundColor: '#3876BF', borderRadius: width / 2, padding: 15 }}>
      ...
      <AnimatedTypewriterText
        sentences={[
          'Procedure for Resonant Breathing',
          '1.Inhale for a count of 5',
          '2.Exhale for a count of 5',
          '3.Continue this breathing pattern for atleast 5 minutes',
          'Repeat the Step!',
        ]}
        delay={1000}
        speed={70}
